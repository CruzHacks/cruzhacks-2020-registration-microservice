...
/*
    ...
    Also: if salt rounds are > 20~ it takes 30seconds - 5 minutes to has
*/
...
const recordExist = async (req) => {
    knex(req.query.type).where('email', req.body.email).then((exist) => {
...
const insert = async (req) => {
    ...
    if (req.query.type == "hacker") {
        // first we check if that email already exist in the hacker table
        const exist = knex('hackers').where('email', req.body.email)
